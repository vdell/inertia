...
why::MouseVelocityTracker::MouseVelocityTracker(int max_samples) : m_max_samples(max_samples)
{

}

void why::MouseVelocityTracker::add_sample(clan::Point pos)
{
	if (pos.x < 0 || pos.y < 0)
	{
		return;
	...
	m_samples[clan::System::get_time()] = pos;
}

void why::MouseVelocityTracker::clear_samples()
{
	m_samples.clear();
}

void why::MouseVelocityTracker::invalidate_samples(clan::ubyte64 time_elapsed_ms_treshold)
{
	...
	{
		const auto td = it->first - m_start_time;
		if (td >= time_elapsed_ms_treshold)
		{
			m_samples.erase(it++);
		}
		else
...
float why::MouseVelocityTracker::get_avg_velocity() const
{
	// Need at least two samples to calculate avg.
	if (m_samples.size() < 2) return 0.0f;

	auto prev_sample = m_samples.end();

	float avg_vel = 0.0f;

	for (auto sample_it = m_samples.begin(); sample_it != m_samples.end(); ++sample_it)
	{
		if (prev_sample == m_samples.end())
		{
			prev_sample = sample_it;
			continue;
		}
		assert(prev_sample != m_samples.end());

		const int dt = static_cast<int>(sample_it->first - prev_sample->first);
		assert(dt > 0);

		avg_vel += (sample_it->second.x - prev_sample->second.x) / dt;

		prev_sample = sample_it;
	}
	return avg_vel / m_samples.size();
}
